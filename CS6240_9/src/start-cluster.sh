...
aws ec2 run-instances --image-id ami-c229c0a2 --count $1 --instance-type r3.xlarge --key-name ec2-key --security-groups ec2-sec-key > instances

# Fetch instanceIds and store
cat instances | jq -r ".Instances[].InstanceId" > instance-ids

# Fetch DNS and store
aws ec2 describe-instances --filters "Name=instance-type,Values=r3.xlarge" | jq -r ".Reservations[].Instances[].PublicDnsName" | sed '/^$/d' > original-dns
